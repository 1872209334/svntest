...
public class AppFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
    	...
    	if (request.getMethod().equals("OPTIONS")){
                ...
                if (headStr.compareTo(jwtConfig.getTokenHead()) == 0) {
                    ...
                    if (claims != null) {
                        ...
                        if( redisUtil.get(userId) != null ){
                        	...
                        	if( redisUtil.get("app_"+userArr[1]).equals(PasswordUtil.createCustomPwd(auth.trim(),jwtConfig.getSecret())) ){
            ...
            response.getWriter().write(JsonUtil.toJson(ReturnUtil.Error("验证失败")));
            return;
    	}
    }
